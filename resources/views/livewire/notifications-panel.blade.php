<div class="relative inline-block" x-data="{open:false}" wire:poll.20s >
    @if($notifications->count() > 0)
        <button class="text-base tracking-tighter text-yellow-500 focus:outline-none" x-on:click="open = !open"
        ...
        <ul x-show="open" x-cloak
            ...
            @foreach($notifications as $noti)
                <li>
                    <a  href="#" 
                        wire:click.prevent="select('{{ $noti->id }}')" 
                        class="block w-full px-2 py-2 text-sm text-left text-gray-800 rounded hover:bg-teal-100"
                    >
                        {{ $noti->data['described'] }}: {{ $noti->data['entityName']}} 
                        <span class="text-xs text-gray-600 font-italic">({{ $noti->created_at->diffForHumans() }})</span>
                    </a>
                </li>
            @endforeach
        </ul>
