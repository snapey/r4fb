            ...
            <div class="flex w-full mb-8 space-x-4">
                <div class="w-4/12">
                    ...
                    <select class="w-full px-1 py-2 text-base leading-tight text-gray-700 border rounded shadow focus:outline-none focus:shadow-outline" name="context">
                        ...
                        <option value="shipment" {{ $template->context == 'shipment' ? 'selected' :''}}>Shipment</option>
                        <option value="allocation" {{ $template->context == 'allocation' ? 'selected' :''}}>Allocation</option>
                    </select>
                    @error('context')
                        <p class="mt-4 text-xs italic text-red-500">{{ $message }}</p>
                    @enderror
                    <h3 class="mt-16 mb-4 text-sm font-bold text-gray-700">Body placeholders:</h3>
                    <div class="p-4 m-2 text-gray-800 bg-gray-100 border rounded shadow">
                        <p class="my-2">Use these tags to insert data into your email templates:</p>
                        <ul>
                            <li>[foodbank] <span class="ml-2 text-gray-600">the name of the foodbank</span></li>
                            <li>[eta] <span class="ml-2 text-gray-600">expected date of delivery</span></li>
                            <li>[satellite] <span class="ml-2 text-gray-600">name of the satelite</span></li>
                        </ul>
                    </div>
