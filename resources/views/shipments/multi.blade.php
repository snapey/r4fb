...
<div class="p-4 bg-gray-200">
    <h1 class="text-xl font-bold text-teal-800 ">Prepare Shipment for Multiple Allocations</h1>

    <div class="flex flex-row">
        <div class="w-6/12 pr-4">
            @foreach($allocations as $allocation)
                <h2 class="mt-4 mb-1 ml-2 overflow-hidden text-base font-bold text-teal-800">Allocation {{ $allocation->id }} - {{ $allocation->foodbank->name ?? ''}}</h2>
                                ...
                                <optgroup label="Foodbank address">
                                    @foreach($allocations as $allocation)
                                        @foreach($allocation->foodbank->addresses as $address)
                                            <option value="{{ $address->id }}">{{ $address->addressable->name }} - {{ $address->address1 }} - {{ $address->postcode }}</option>
                                        @endforeach
                                    @endforeach
                                </optgroup>
                                <optgroup label="Shippers">
