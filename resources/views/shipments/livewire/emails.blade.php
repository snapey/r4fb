        ...
        <div class="container z-50 max-w-4xl mx-auto overflow-y-auto bg-white rounded shadow-lg" style="height: 80vh;">
            ...
            <div class="px-4 py-4 mb-4 text-left modal-content">
                ...
                <div class="flex items-center justify-between pb-3 border-b-2">
                    <p class="text-xl font-bold">Shipment Email Centre</p>
                ...
                <div class="flex flex-row text-sm text-gray-800">
                    ...
                    <div class="w-6/12">
                        ...
                        <h3 class="my-4 text-xl font-bold">From:</h3>
                        @each('shipments.livewire._contact',$shipment->fromAddress->addressable->contacts,'contact')

                        <h3 class="my-4 text-xl font-bold">To:</h3>
                        @each('shipments.livewire._contact',$shipment->toAddress->addressable->contacts,'contact')

                        <h3 class="my-4 text-xl font-bold">Foodbanks:</h3>
                        @foreach($shipment->allocations as $allocation)
                            <div class="font-bold text-teal-800">{{ $allocation->foodbank->name ?? '??????'}}</div>
                            @each('shipments.livewire._contact',$allocation->foodbank->contacts,'contact')
                        @endforeach

                        <h3 class="my-4 text-xl font-bold">Rotary Clubs:</h3>
                        @foreach($shipment->allocations as $allocation)
                            @foreach($allocation->foodbank->clubs as $club)
                                <div class="font-bold text-teal-800">{{ $club->name ?? '??????'}}</div>
                                @each('shipments.livewire._contact',$club->contacts,'contact')
                            @endforeach
                        @endforeach
                    ...
                    <div class="w-6/12 pl-8">
                        <label class="block w-full px-2 py-2 text-sm font-bold" for="template_id">Choose a template:</label>
                        <select 
                        ...
                        <div class="w-full mt-4">
                            <label for="subject" class="block mx-2 mb-2 text-sm font-bold text-gray-800">Subject:</label>
                            <input wire:model="subject" id="subject" type="text" name="subject"
                                class="text-base shadow border rounded w-full py-1 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline 
